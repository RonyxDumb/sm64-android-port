...
int test_vsync(void) {
#ifdef __ANDROID__
    /*
     * Android startup anti-stutter:
     *
     * The original code tried to estimate the refresh rate by performing
     * many consecutive SDL_GL_SwapWindow() calls during startup. On Android
     * this can be noisy because SurfaceFlinger / the display mode may still
     * be settling, producing a wrong render_multiplier on some launches.
     *
     * Instead, ask SDL for the active display mode and snap the reported
     * refresh rate to a sane/common value.
     */
    SDL_DisplayMode mode;
    int refresh_rate = 60;
    ...
    } else if (SDL_GetDesktopDisplayMode(0, &mode) == 0 && mode.refresh_rate > 0) {
        refresh_rate = mode.refresh_rate;
    }

    /*
     * Normalize noisy/rounded Android values:
     * 59/60/61 -> 60
     * 89/90/91 -> 90
     * 119/120/121 -> 120
     * 29/30/31 -> 30
     */
    if (refresh_rate >= 28 && refresh_rate <= 32) {
        refresh_rate = 30;
    } else if (refresh_rate >= 55 && refresh_rate <= 65) {
        refresh_rate = 60;
    } else if (refresh_rate >= 85 && refresh_rate <= 95) {
        refresh_rate = 90;
    } else if (refresh_rate >= 115 && refresh_rate <= 125) {
        refresh_rate = 120;
    }

    /*
     * This port uses render_multiplier to keep the 30 Hz simulation clock
     * correct while presenting once per physical refresh.
     *
     * 30 Hz  -> 1
     * 60 Hz  -> 2
     * 90 Hz  -> 3
     * 120 Hz -> 4
     */
    render_multiplier = (refresh_rate + 15) / 30;

    if (render_multiplier < 1) {
        render_multiplier = 1;
    }

    /*
     * Prevent pathological values if Android reports an unusual mode.
     * Four swaps are enough for displays up to 120 Hz.
     */
    if (render_multiplier > 4) {
        render_multiplier = 4;
    }

    vsync_enabled = (SDL_GL_SetSwapInterval(1) == 0);

    printf("Android display refresh: %d Hz, render_multiplier: %d, VSync: %s\n",
           refresh_rate,
           render_multiplier,
...
#else
    /*
     * Desktop fallback: retain the original empirical VSync detection.
     */
    Uint32 start;
    Uint32 end;
    ...
    float average = 4.0f * 1000.0f / (float)(end - start);
...
static void sync_framerate_with_timer(void) {
    ...
    const Uint64 frame_ticks = frequency / 30;
    ...
    if (now < next_frame) {
        Uint64 remaining = next_frame - now;
        Uint32 delay_ms = (Uint32)((remaining * 1000ULL) / frequency);
        ...
        /*
         * App resume / debugger / long stall:
         * reset the deadline instead of trying to catch up.
         */
        next_frame = now;
    }
